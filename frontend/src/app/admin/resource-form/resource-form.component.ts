...
export class ResourceFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.resourceForm
      .get('type')
      ?.valueChanges.subscribe((type: ResourceType) => {
  ...
  private updateFormValidators(resourceType: ResourceType): void {
    ...
    fields.forEach((field) => {
      this.resourceForm.get(field)?.clearValidators();
    ...
    switch (resourceType) {
      ...
      case ResourceType.AUDIOBOOK:
        ...
        this.resourceForm.get('publisher')?.setValidators([]);
        this.resourceForm
          .get('publishedYear')
          ?.setValidators([
            Validators.min(1000),
        ...
        this.resourceForm.get('pageCount')?.setValidators([Validators.min(1)]);
        break;
      ...
      case ResourceType.DVD:
        this.resourceForm.get('director')?.setValidators([Validators.required]);
        this.resourceForm.get('actors')?.setValidators([]);
        this.resourceForm.get('duration')?.setValidators([Validators.min(1)]);
        this.resourceForm
          .get('publishedYear')
          ?.setValidators([
            Validators.min(1900),
      ...
      case ResourceType.GAME:
        this.resourceForm
          .get('developer')
          ?.setValidators([Validators.required]);
        this.resourceForm.get('platform')?.setValidators([Validators.required]);
        this.resourceForm
          .get('pegiRating')
      ...
      case ResourceType.MAGAZINE:
        this.resourceForm
          .get('publisher')
          ?.setValidators([Validators.required]);
        this.resourceForm.get('issueNumber')?.setValidators([]);
        this.resourceForm.get('frequency')?.setValidators([]);
        this.resourceForm.get('pageCount')?.setValidators([Validators.min(1)]);
        break;
    }
    ...
    this.bookService.addBook(resourceData, coverFile).subscribe({
      ...
      error: (error) => {
        ...
        if (
          ...
          this.resourceForm.get('isbn')?.setErrors({ duplicateIsbn: true });
        } else {
          this.notificationService.error(
